...
void FeatureExtractor::parseRowsFromOS1CloudMsg(const sensor_msgs::PointCloud2::ConstPtr &cloud, pcl::PointCloud<ouster_ros::Point>::Ptr &cloud_raw,
                                                pcl::PointCloud<PointType>::Ptr &cloud_filt, std::vector<int> &rows_start_indices,
                                                std::vector<int> &rows_end_indices, std::unordered_map<unsigned int, unsigned int> &indices_in_raw_cloud,
                                                float &processing_time) {
  TicToc t_prepare;
